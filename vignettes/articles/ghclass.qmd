...
wait_until = function(cond, tries = 12, delay = 5) {
...
key = system2("op", c("read", shQuote("op://Employee/ghclass credentials key/password")), stdout = TRUE)
...
repos = org_repos("ghclass-vignette")
teams = org_teams("ghclass-vignette")
users = org_members("ghclass-vignette", include_admins = FALSE)

repo_delete(repos, prompt = FALSE)
team_delete("ghclass-vignette", teams, prompt = FALSE)
org_remove("ghclass-vignette", users, prompt = FALSE)
wait_until(function() length(org_members("ghclass-vignette", include_admins = FALSE)) == 0)

repo_set_template("Sta323-Sp19/hw1", FALSE)
org_set_permissions(
  "ghclass-vignette", repo_permission = "read", create_repositories = TRUE, create_teams = TRUE
)
```
...
This article is about the nitty-gritty of setting your class up in GitHub with
ghclass.
For a higher level discussion of why and how to use Git/GitHub in data
science education, see [this paper](https://doi.org/10.1080/00031305.2017.1397549) by the
package authors.
...
Additionally, they can adjust repo and team memberships by adding or removing collaborators to assignment repositories as well as deleting them.
...
This can help with minimizing accidents that cannot be undone and makes sure students cannot peek into each other's repositories unless you explicitly allow them to do so.
...
Then, throughout the semester you will need to create a total of 570 repositories.
...
That being said, ghclass does facilitate setting up continuous integration tools using GitHub Actions for students' assignment repos (see below).
This can allow for some automatic checking and feedback each time students push to a repo.
...
credentials stored with `gitcreds::gitcreds_set()`, see `?github_token` for details.
...
Personal access tokens > Tokens (classic)) or use
[`usethis::create_github_token()`](https://usethis.r-lib.org/articles/git-credentials.html).
When creating the token make sure it has the `repo` and `admin:org` scopes, along
with `workflow` if you plan to add GitHub Actions workflows to student repos and
`notifications` if you plan to use `repo_watch()`, `repo_ignore()`, or
`repo_unwatch()`, and `delete_repo` if you plan to use `repo_delete()`. Note that
`usethis::create_github_token()` does not select `admin:org` by default.
These scopes apply to classic tokens. Fine-grained tokens can also be used, but
GitHub does not report their permissions so `github_token_sitrep()` cannot
check them, and GitHub does not support them for `repo_watch()`, `repo_ignore()`,
or `repo_unwatch()`.
...
```{r}
#| label: bad_token
github_test_token("badtoken")
```

`github_token_sitrep()` provides a more detailed report on the token, including
its type, source, and granted scopes, and flags any of the scopes above that are
missing:
```{r}
#| label: token_sitrep
github_token_sitrep()
...
ghclass can also be used with a GitHub Enterprise Server instance by setting the
`GITHUB_API_URL` environment variable to the instance's REST API endpoint, e.g.
`https://github.example.edu/api/v3`, typically in a project-level `.Renviron`.
All API requests, clone and push URLs, and badge links will then use that host.
Tokens are looked up per host, so a token for github.com and one for your
Enterprise instance can be stored side by side with
`gitcreds::gitcreds_set("https://github.example.edu")`.

## Step-by-step guide

Start with creating an organization on GitHub for the course. We recommend using the course number, semester/quarter, and year in the organization name, e.g. for a course numbered Sta 199 in Spring 2026, you can use something like `Sta199-Sp26`.
...
Private repositories have been free for all GitHub users and organizations since 2019, so no special discount or plan is needed to keep student work private.
GitHub Education does still offer [benefits for verified teachers](https://github.com/education/teachers), including a free upgrade of your course organization to the GitHub Team plan.

This is an optional step, but one that many will want to do.
...
Each new GitHub organization defaults to repositories being readable by all members, regardless of whether they are private or public. This is clearly undesirable for most classroom settings.

Individual-level permissions can be set via the "People" tab on the
organization page. We recommend that the course instructor and any teaching
assistants be owners of the organization and that students be members.
...
We can get a quick snapshot of our organization using the `org_sitrep` function which reports on these settings as well as other important details.
...
We can see that the current default repository permission setting is "read", with a helpful warning that this enables members to view all repositories, and that members are currently able to create repositories and teams.
These settings can be changed within the Organization Settings page under Member privileges. Alternatively, we can use ghclass to apply the member repository and team settings directly with `org_set_permissions` (the default repository permission on its own can also be changed with `org_set_repo_permission`).
The admin repository permissions (visibility change, deletion and transfer, and issue deletion) are read-only in the GitHub API, so these still need to be set on the settings page.
...
After changing these settings we can once again check the org's sitrep and see that the warning is now resolved.
...
The sitrep also reports the organization's default workflow permissions for GitHub Actions, which can be changed with `org_set_workflow_permissions`.

### Adding students to the organization

Next, collect your students' GitHub usernames. You can do this using your web form tool of choice (e.g. Google Forms, MS Forms, etc.) or via a quiz or survey on your school's learning management system (LMS). We will assume that you are able to then read these data into an R data frame.
...
roster
```
Here we are using the students' school email addresses as unique identifiers. We also have their GitHub usernames, and we have assigned them to different teams for our three homework assignments.
If you would rather have ghclass form the teams for you, `team_roster` randomly assigns the members of a roster to teams of a given size and adds the team names as a new column.
...
wait_until(function() !any(names(pats)[c(1,3,6)] %in% org_pending("ghclass-vignette")))
```
...
wait_until(function() length(org_pending("ghclass-vignette")) == 0)
```
...
Generally, this will consist of things like a `README.md` with instructions, a scaffolded `qmd`, `Rmd`, or `R` file where the students will enter their answers, and any other necessary support files (e.g. data, images, support scripts, etc.).
...
We can view the contents of a repository without cloning it using `repo_tree`,

```{r}
#| label: repo_tree
repo_tree("Sta323-Sp19/hw1")
```

Once you have created the repository and are ready to distribute it to students there is one more suggested step - setting the repo's template status to `TRUE`.
...
```{r}
#| include: false
wait_until(function() repo_is_template("Sta323-Sp19/hw1"))
```
...
Once you have created your template repository, it is then a straightforward process to create the team or individual repositories for your students.
...
Once the repos are created we can interact with them using ghclass. One of the most common needs is simply to list which repos exist within our organization and/or select some subset of them.
...
Let's assume that we distributed hw1 with the wrong version of the `README.md` included. If we want to replace this with the correct version across all of the hw1 repositories, we could do the following:

```{r}
#| label: add_file
file = system.file("hw1/README.md", package = "ghclass")
...
Eventually the students will be finished with the work and/or the assignment deadline will have passed. ghclass makes it easy to collect all of the student work from GitHub and make it accessible on your local computer for grading. We make use of the [gert](https://github.com/r-lib/gert) package to provide basic git functionality within ghclass.
...
If your template repository includes a workflow under `.github/workflows/`, for example one that renders the students' document or runs a set of checks, then GitHub Actions will run it each time a student pushes to their repository.
ghclass provides several tools for working with these workflows across many repositories:

- `org_create_assignment(add_badges = TRUE)` adds a status badge for each workflow to the README of every assignment repo so students can see whether their most recent push succeeded (see also `action_add_badge` and `action_remove_badge`).
- `action_workflows` and `action_runs` report on the workflows and their runs across repos, and `action_runtime` reports how long the runs took.
- `action_artifacts` and `action_artifact_download` list and download the artifacts produced by the workflows, e.g. rendered html or pdf output.
- `org_grade_assignment` combines these steps for grading: it clones all repos matching a filter, downloads the requested artifacts (skipping any that are out of sync with the current commit), and creates a comment file for each repo.

For example, to set up grading for hw1 and collect the rendered html artifacts from each repo:
...
org_grade_assignment(
  ...
  artifacts = c("html" = "html-output")
)
```

Note that adding or modifying workflow files in student repos requires the `workflow` scope on your token.
...
\
GitHub retired GitHub Classroom on August 28, 2026 (see [here](https://github.com/orgs/community/discussions/205975) for the announcement). Organizations, repositories, and user accounts created through Classroom were not affected, so existing classroom organizations can continue to be managed with ghclass, which works directly with the GitHub API. The workflow described above covers the distribution of individual and team assignments, and GitHub Actions workflows can provide the automatic checking and feedback that Classroom's autograding did.

3. Does the default branch of my repository matter (`master` vs `main`)? \
\
Yes and no - since October 2020 GitHub has used `main` as the default branch for new repositories (see [here](https://github.com/github/renaming) for background), while repositories and organizations created before then kept `master` unless they were changed. ghclass supports either name across the entire package. For the vast majority of use cases you will not see any differences as the GitHub API and/or Git will already use the default branch without any additional specification. In a small number of cases a branch name is required, in which case the package does not provide a default value and you will be prompted to specify that argument.\
\
If your classroom org is already using `master` you can leave it as is, particularly for repos already distributed to students. For new organizations, the default branch name for newly created repositories can be set under `Org > Settings > Repository Defaults > Repository default branch`. Note that this only affects newly created repositories, not existing ones.
