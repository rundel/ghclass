...
If this is your first time setting up a personal access token (PAT), generate a token in the browser after logging into Github (Settings > Developer Settings > Personal access tokens) or use [`usethis::browse_github_token`](https://usethis.r-lib.org/articles/articles/usethis-setup.html#get-and-store-a-github-personal-access-token). Copy your token to the clipboard and paste it as a character string into the `set_github_token` function.
...
Next, collect your students' GitHub user names. You can do this with 
...
columns: one called `github` which contains students' GitHub user names and another column called `team` that contains the team 
...
Using the `roster` data frame, we can then invite the students to the class' organzation. This will send and email to each student inviting them to join the GitHub organization. 
...
We use the variable `team` within the `roster` data frame to create teams within the organization and add students to their teams. 

```r
create_team("Sta523-Fa17", unique(roster$team))

## Adding Team1 to Sta523-Fa17 ...
## Adding Team2 to Sta523-Fa17 ...
## Adding Team3 to Sta523-Fa17 ...
## Adding Team4 to Sta523-Fa17 ...
## Adding Team5 to Sta523-Fa17 ...
```
...
You can check which students are added to the organization using the `get_members` function, and pending invitations using the `get_pending_members` function. Note that if students are first-time GitHub users, they will need to verify the email address associated with their GitHub account and accept the invitation before they show up as a member of the organization.

### Creating a team assignment

First, create repositories for each team with a prefix identifying the 
assignment. The resulting repo names will be this prefix followed by the 
team name.

```r
create_team_repo("Sta523-Fa17", unique(roster$team), prefix="hw2-")

## Creating repo Sta523-Fa17/hw2-Team01 ...
## Creating repo Sta523-Fa17/hw2-Team02 ...
## Creating repo Sta523-Fa17/hw2-Team03 ...
## Creating repo Sta523-Fa17/hw2-Team04 ...
## Creating repo Sta523-Fa17/hw2-Team05 ...
```
...
<!-- This part needs to be changed based on the discussion regarding GH issue #16 -->

You can modify repos after they have been created. This will overwrite 
