...
Using the `roster` data frame, we can then invite the students to the class' organzation. This will send and email to each students inviting them to join the GitHub organization. If students are first-time GitHub users, they will need to verify the email address associated with their GitHub account and accept the invitation before they show up as a member of the new organization.
...
We can use the variable `team` within the `roster` data frame to create teams within the organization and add students to their teams. 
...
You can check which students are added to the organization using the `get_members`, and pending invitations using the `get_pending_members`, functions respectively.
