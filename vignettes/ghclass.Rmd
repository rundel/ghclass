...
We now need to wait for the students to accept these invitations before the will have access to the organization. We can check the status of these acceptances using the `org_members()` and `org_pending_members()` to see which students have accepting and have not accepted the invitation.

```{r}
org_members("ghclass-vignette")
org_members("ghclass-vignette", include_admins = FALSE)
org_pending_members("ghclass-vignette")
```
...
We will now create teams which will be used for the first two homework assignments in the class. We can either use the `hw01` and `hw02` columns from the `roster` data frame directly or we can vectorize the process and add either a common prefix or suffix.

```{r}
team_create("ghclass-vignette", roster$hw01)
```

```{r}
team_create("ghclass-vignette", team= c("team01", "team02","team03"), prefix="hw02-")
```

Once the teams are created we can then add the students to these teams.

```{r}
team_invite(org = "ghclass-vignette", user = roster$github, team = roster$hw01)
team_invite(org = "ghclass-vignette", user = roster$github, team = roster$hw02)
```
...
```{r}
team_invite("ghclass-vignette", roster$github, roster$hw03)
```
...
```{r}
r = repo_create("ghclass-vignette", name = c("team01", "team02","team03"), prefix="hw01-")
repo_add_team(r, roster$hw01)
```
...
```{r}
teams = org_teams("ghclass-vignette", filter="hw01-", exclude = TRUE)

repo_create("ghclass-vignette", teams)

repo_add_team(org_repos("ghclass-vignette","hw02-"), roster$hw02)

repo_add_team(org_repos("ghclass-vignette","hw03-"), roster$hw03)
```
...
repo_mirror(
  ...
  target_repo = org_repos("ghclass-vignette","hw01-")
)
```
...
repo_mirror(
  ...
  target_repo = org_repos("ghclass-vignette","hw04-")
)
```
...
repo_add_file(
  org_repos("ghclass-vignette","hw01-"),
...
```{r}
cat( repo_get_readme("ghclass-vignette/hw01-team01") )
```
...
```{r}
local_repo_clone(
  repo = org_repos("ghclass-vignette", "hw01-"),
  local_path = "hw01"
)
