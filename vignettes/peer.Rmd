...
# Before peer review
Below, we set up a set of repositories for a fictional assignment "hw2" for four students using the `create_repo()` function. For a more detailed description of how to set up student repositories in a GitHub Organization, please see the `ghclass` vignette.
```{r}
library(ghclass)
devtools::load_all()
...
- The `rev*` column(s) specify the randomized user ID of the student(s) that should review the user's assignment.

We recommend for instructors to create a new roster file (with a new random seed) for each assignment to prevent the same pairings of students reviewing each others work over and over throughout the course.
```{r}
...
```{r}
peer_create_rform(2,
                  file = "/Users/thereseanders/Documents/RStudio/introggplot2/hw2/rev_to_aut_feedback_blank.Rmd")
```
...
```{r}
peer_create_aform(file = "/Users/thereseanders/Documents/RStudio/introggplot2/hw2/aut_to_rev_feedback_blank.Rmd", 
                  category = c("accuracy", "fairness"))
```


### [Recommended] Distribute author rating forms
Instructors may customize the rating forms before distributing them to the authors.
```{r}
peer_add_file_aut(
        org = "ghclass-test",
        roster = roster_test,
...
The function `peer_return()` places the author's original file in review-specific folder on authors' repositories and then copies reviewer files into that folder. The function also opens an issue to alert authors that the review process is complete. The issue contains a link to the commit that illustrates the difference between their assignment and reviewers' suggestions, the written feedback from reviewers, and if applicable, the form requesting their rating on the reviewers' feedback.
...
Only select files will be moved and saved in a new folder in authors' repositories and should be specified by the instructor via the `path` parameter. Please not that the `peer_return()` function is relatively costly in terms of calls to the API. Instructors are advised to select only the most relevant files in this step, for example the review feedback form and comments made by the reviewer on authors' original files.
```{r}
peer_return(org = org,
            roster = roster_test,
            path = c("hw2_task.Rmd"),
...
```{r}
ascores = peer_score_rating(
  org = "ghclass-test",
  roster = roster_test,
  form_rating = "afeedback_blank.Rmd",
  dblind = dblind,
  prefix = prefix,
  write_csv = F
)
...
### Timing
Instructors should carefully consider the timing of deadlines for a) the assignment, b) the completion of the review, and c) the completion of the author rating of reviewers' feedback. 
